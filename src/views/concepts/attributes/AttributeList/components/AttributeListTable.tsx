...
const AttributeListTable = ({ className }: AttributeListTableProps) => {
    ...
    // 🔹 Pagination + Sorting + Search birlikte yönetiliyor
    ...
    // 🔍 Arama inputu ve debounce state
    const [searchInput, setSearchInput] = useState('')
    const [debouncedSearch, setDebouncedSearch] = useState('')

    // ⏳ 300ms debounce efekti
    useEffect(() => {
        const timeout = setTimeout(() => {
            ...
            setPagination((prev) => ({
                ...
                start: 0, // arama yapıldığında sayfa 1'e dön
            }))
        }, 300)
        return () => clearTimeout(timeout)
    }, [searchInput])

    // 🔹 React Query: Attribute List
    const {
        ...
        async () => {
            ...
            const response =
                await attributeApi.apiAttributeAttributeListPost(requestData)
    ...
    const deleteAttributeMutation = useMutation({
        mutationFn: async (deleteCommand: AttributeDeleteCommand) => {
            await attributeApi.apiAttributeDeleteDelete(deleteCommand)
        },
        onSuccess: () => {
            toast.push(
    ...
    const onEdit = (attribute: AttributeGridListResponseDto_Item) => {
        navigate(`/concepts/attributes/attribute-edit/${attribute.id}`)
    }
    ...
    const columns: ColumnDef<AttributeGridListResponseDto_Item>[] = useMemo(
        () => [
            {
                header: t('nav.conceptsAttributes.name'),
            ...
            {
                header: t('nav.conceptsAttributes.accept_text'),
                accessorKey: 'willAcceptTextValues',
                enableSorting: true,
                cell: (props) => {
                    const val = props.getValue() as boolean
                    return val
                        ? t('nav.conceptsAttributes.yes')
                        : t('nav.conceptsAttributes.no')
                },
            },
            {
                header: t('nav.conceptsAttributes.visible_tree'),
            ...
            >
                <div className="flex flex-col justify-between">
                    <div className="px-6 py-4">
                        <h5 className="mb-4">
                            {t('nav.conceptsAttributes.delete_attributes')}
                        </h5>
                        <p>
                            {t('nav.conceptsAttributes.are_you_sure')} "
                            {selectedAttribute?.attributeName}"?{' '}
                            {t('nav.conceptsAttributes.connot_undone')}
                        </p>
                    </div>
